...
const uint32_t MAX_ARRAY_VALUES = 256;
...
bool array_logical_range_ok(int32_t logical_minimum, int32_t logical_maximum) {
    return logical_maximum >= logical_minimum &&
           (uint32_t) ((int64_t) logical_maximum - logical_minimum + 1) <= MAX_ARRAY_VALUES;
}
        ...
        switch (item) {
            case HID_INPUT: {
                ...
                if ((value & 0x03) == 0x02) {  // scalar
                    ...
                    if (!array_logical_range_ok(logical_minimum, logical_maximum)) {
                        bitpos[report_id] += report_size * report_count;
                    } else if (usage_minimum && usage_maximum) {
                        uint32_t usage = usage_minimum;
            ...
            case HID_LOGICAL_MAXIMUM:
                ...
                logical_maximum = logical_minimum < 0 ? sign_extend(value, item_size) : (int32_t) value;
                break;
